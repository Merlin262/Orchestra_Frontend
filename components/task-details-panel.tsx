...
export default function TaskDetailsPanel({ task }: TaskDetailsPanelProps) {
  ...
  <div className="flex justify-between items-center p-6 border-b border-gray-200 dark:border-gray-700">
    <div className="flex items-center gap-3">
      ...
      ></div>
      <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{task.name}</h2>
    </div>
    <div className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 px-3 py-1 rounded-full text-sm">
  ...
  <div className="border-b border-gray-200 dark:border-gray-700">
    <nav className="flex">
      ...
      >
        Tarefas Pendentes
        <span className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 px-2 py-0.5 rounded-full text-xs">
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Progresso</h3>
            <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{task.details.progress}%</span>
          </div>
        ...
        <div className="flex items-start gap-4">
          <User size={20} className="text-gray-400 dark:text-gray-500 mt-0.5" />
          <div>
        ...
        {/* Datas */}
        <div className="flex items-start gap-4">
          <Calendar size={20} className="text-gray-400 dark:text-gray-500 mt-0.5" />
          <div>
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Datas</h3>
            <div className="mt-1 grid grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">Início</p>
                <p className="text-sm text-gray-900 dark:text-gray-100">{task.details.startDate}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">Prazo</p>
                <p className="text-sm text-gray-900 dark:text-gray-100">{task.details.dueDate}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Descrição */}
        <div className="flex items-start gap-4">
          <MessageSquare size={20} className="text-gray-400 dark:text-gray-500 mt-0.5" />
          <div>
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Descrição</h3>
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">{task.details.description}</p>
          </div>
        </div>

        {/* Conclusão de subtarefas */}
        <div className="flex items-start gap-4">
          <CheckCircle size={20} className="text-gray-400 dark:text-gray-500 mt-0.5" />
          <div>
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Conclusão de Tarefas</h3>
            <div className="mt-1">
      ...
      <div>
            ...
            <li key={subtask.id} className="py-3">
              <div className="flex items-start">
                ...
                <div className="ml-3 flex-1">
                  <p className={`text-sm ${subtask.completed ? "text-gray-500 dark:text-gray-400 line-through" : "text-gray-900 dark:text-gray-100"}`}>
                    {subtask.name}
                  </p>
                </div>
                <div>
        ...
        {task.details.subtasks.length === 0 && (
          <div className="text-center py-6">
            <p className="text-gray-500 dark:text-gray-400">Nenhuma tarefa pendente encontrada</p>
          </div>
        )}

        {/* Adicionar nova subtarefa */}
        <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Adicionar nova subtarefa</h3>
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="Nome da subtarefa"
              className="flex-1 border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
            />
      ...
      <div>
        <ul className="space-y-4">
          {task.details.comments.map((comment: any, index: number) => (
            <li key={index} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
              <div className="flex justify-between items-start">
                <span className="font-medium text-gray-900 dark:text-gray-100">{comment.author}</span>
                <span className="text-xs text-gray-500 dark:text-gray-400">{comment.date}</span>
              </div>
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">{comment.text}</p>
            </li>
          ))}
        </ul>
  ...
  {/* Rodapé */}
  <div className="border-t border-gray-200 dark:border-gray-700 p-4 flex justify-between items-center bg-white dark:bg-gray-900">
    ...
    <div className="flex gap-2">
      ...
      <button className="px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-md text-sm transition-colors">
        Reassignar
      </button>
    </div>
  </div>
